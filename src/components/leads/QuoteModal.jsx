...
export default function QuoteModal({ lead, accountId, onSubmit, onClose }) {
  const [form, setForm] = useState({ amount: "", currency: "ILS", status: "טיוטה", notes: "" });
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const handleSubmit = () => {
    onSubmit({ ...form, lead_id: lead.id, amount: parseFloat(form.amount) || 0, account_id: accountId });
  };
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>הצעת מחיר ל{lead.full_name || lead.phone}</DialogTitle>
        </DialogHeader>
